...
Y_test = Y2;
%% Normalization
[inputn,inputps]=mapminmax(X_train', -1, 1);
[outputn,outputps]=mapminmax(Y_train', -1, 1);
inputn_test = mapminmax('apply',X_test',inputps); 
%% construct the QRGRU network
tau = [0.05, 0.025, 0.01];
for i = 1:3
alpha = tau(i);
% parameter settings
...
options = trainingOptions('adam', ...
   ...
   YPred1 = mapminmax('reverse',YPred1, outputps);
   PRE1(:,i) = YPred1';
...
writetable(T, 'QRGRU_SPX_Add.xlsx');
writetable(T2, 'QRGRU_SPX_Mult.xlsx');
